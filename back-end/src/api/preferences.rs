use crate::{
    db::{preferences as pref_db, courses as course_db},
    errors::AppError,
...
async fn update_preferences_handler(
    ...
    log::info!("Updating application preferences - setting current course ID to: {}", payload.current_course_id);

    let course_id = Uuid::parse_str(&payload.current_course_id)
        .map_err(|_| AppError::BadClientData("Invalid current_course_id format. Expected UUID.".to_string()))?;
    ...
    log::info!("Successfully set current course ID preference to: {}", course_id);
