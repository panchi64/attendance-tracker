...
async fn generate_and_store_code(
    ...
    let code: String = rng()
        ...
        .map(char::from)
        .collect::<String>()
        .to_uppercase(); // Make it uppercase for easier entry
...
pub async fn validate_code(
    ...
    let code_details = course_db::fetch_course_code_details(pool, course_id).await?
        .ok_or_else(|| AppError::NotFound(format!("Course {} not found during code validation", course_id)))?;

    match code_details {
        (Some(stored_code), Some(expires_at_naive)) => {
            let expires_at_utc: DateTime<Utc> = DateTime::from_naive_utc_and_offset(expires_at_naive, Utc);
...
pub fn start_confirmation_code_generator(pool: SqlitePool, interval_duration: Duration) {
    ...
    tokio::spawn(async move {
        ...
        interval.tick().await; // Initial tick immediately (or shortly after start)
                ...
                Ok(courses) => {
                    ...
                    for course in courses {
                        match generate_and_store_code(&pool, course.id, interval_duration).await {
                            Ok(new_code) => {
                                log::trace!("Generated new code {} for course {}", new_code, course.name)
                            }
                            Err(e) => {
                                log::error!("Failed to generate code for course {}: {}", course.name, e)
                            }
                        }
                    }
                }
...
pub async fn get_current_code(pool: &SqlitePool, course_id: Uuid) -> Result<Option<(String, NaiveDateTime)>, AppError> {
    let code_details = course_db::fetch_course_code_details(pool, course_id).await?
        .ok_or_else(|| AppError::NotFound(format!("Course {} not found when getting current code", course_id)))?;

    match code_details {
        (Some(code), Some(expires_naive)) if expires_naive > Utc::now().naive_utc() => Ok(Some((code, expires_naive))), // Compare NaiveDateTime
        _ => Ok(None), // No valid code exists or it's expired
    }
}